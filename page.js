...
const movConverter = () => {
    ...
    const transcode = async () => {
        ...
        try {
            ...
            await ffmpeg.exec(['-i', 'input', 'output.mov']);
            
            const data = await ffmpeg.readFile('output.mov');
            const videoURL = URL.createObjectURL(new Blob([data.buffer], { type: 'video/quicktime' }));
            setDownloadLink(videoURL);

            // Automatically trigger download
            triggerDownload(videoURL, 'output.mov');
        } catch (error) {
            ...
            {loaded && (
                <div className="actions">
                    {processing ? (
                        ...
                        <button className="merge-btn" onClick={transcode}>Convert to MOV</button>
                    )}
                </div>
            )}
        </div>
